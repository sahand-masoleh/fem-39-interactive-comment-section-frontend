...
function Article() {
	...
	const articleObserver = useRef(new IntersectionObserver(handleIntersect));
	...
	function handleIntersect(event) {
		setIsVisible(event[0].isIntersecting);
	}

	return (
		<>
			<article id="article" className="article" ref={articleRef}>
				...
				<details>
					...
					<p className="article__text article__text--small">
						...
						used some quotes from famous people and randomly assigned
						parent-child relations, votes, and dates to them. These people have
						not used this app to post a comment and/or vote on each other's
						posts, and I am confident that most of them do not use Github and
						therefore cannot even log in.
					</p>
				</details>
...
function GoUp() {
	...
	return createPortal(
		<div className="go-up-wrapper">
			<button className="go-up" aria-label="Go to top" onClick={handleClick}>
				<ArrowUpIcon />
			</button>
		</div>,
		document.body
